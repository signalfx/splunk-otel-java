...
| -------------------------------------- | -------------------------------------- | ----------------------- | ------- | -------                                                                                                                                                                                                                                                          |
| `splunk.access.token`                  | `SPLUNK_ACCESS_TOKEN`                  | unset                   | Beta  | (Optional) Auth token allowing exporters to communicate directly with the Splunk cloud, passed as `X-SF-TOKEN` header. Currently the [both Jaeger and OTLP trace exporters](#trace-exporters) and [SignalFx metrics exporter](metrics.md) support this property.
| `splunk.metrics.enabled`               | `SPLUNK_METRICS_ENABLED`               | `false`                 | Beta    | Enables exporting metrics. See [this document](metrics.md) for more information.
| `splunk.metrics.endpoint`              | `SPLUNK_METRICS_ENDPOINT`              | `http://localhost:9943` | Beta    | The SignalFx metrics endpoint to connect to.
| `splunk.metrics.export.interval`       | `SPLUNK_METRICS_EXPORT_INTERVAL`       | `30000`                 | Beta    | The interval between pushing metrics, in milliseconds.
| `splunk.trace-response-header.enabled` | `SPLUNK_TRACE_RESPONSE_HEADER_ENABLED` | `false`                 | Beta    | Enables adding server trace information to HTTP response headers. See [this document](server-trace-info.md) for more information.
