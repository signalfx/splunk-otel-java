...
- **What happens when exporter endpoint is temporarily unreachable?** Sending telemetry will resume when endpoint
  becomes available again. Telemetry collected meanwhile will get dropped. To avoid dropping data it is recommended to
  use [Splunk OpenTelemetry Connector](https://github.com/signalfx/splunk-otel-collector) that supports retrying failed
  transmissions.